...
// LocalHTTPServer exposes push streams as local HTTP GET endpoints.
//
// Endpoints:
//
//	GET  /stream/{name}        — stream bytes as chunked response
//	GET  /stream/{name}/offset — current receive offset (JSON)
//	GET  /streams              — list subscribed streams + offsets
//	GET  /health               — liveness
...
func (h *LocalHTTPServer) Run() error {
	...
	mux.HandleFunc("/streams", func(w http.ResponseWriter, r *http.Request) {
		result := make(map[string]int64, len(h.sess.pushSubs))
		for _, name := range h.sess.pushSubs {
